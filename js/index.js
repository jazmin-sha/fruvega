(function () {
  ...
  function getPriceInfo(product, countryKey, weight) {
    let price =
      (product.prices[countryKey] && product.prices[countryKey][weight]) ||
  ...
  function renderProducts(products, countryKey) {
    const gridRows = document.querySelectorAll(".ltn__product-grid-view .row");
    const listRows = document.querySelectorAll(".ltn__product-list-view .row");
  ...
  async function processProductPrices() {
      ...
      buttons.forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          const filtered =
            selected === "All"
              ? pricesData.products
              : pricesData.products.filter((p) => p.category === selected);
    ...
    function setProductWeight(weight) {
      ...
      document
        .querySelectorAll(`#product-title[data-product="${product.id}"]`)
        .forEach((span) => {
          span.textContent = productName;
        });
      document
        .querySelectorAll(`.original_price[data-product="${product.id}"]`)
        .forEach((span) => {
